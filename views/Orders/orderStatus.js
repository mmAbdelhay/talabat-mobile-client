    ...
    render(){
      return (
        ...
        <View style={styles.container}>
            <Text style={styles.bigtext}>{this.state.currentGif}{this.state.currentState!=4?"...":""}</Text>
            <Image
            source={{ uri: `${ServerIP}/orderstatus/images/${this.state.currentGif}.gif` }}
            style={{ width: 250, height: 250 }}
            />
            <Text style={styles.text}>{this.state.currentState==0?"Waiting for restaurant to approve order.":
            (this.state.currentState==1?"Order is being prepared.":
            (this.state.currentState==2?"Order is ready for delivering.":
            (this.state.currentState==3?"Order is on its way.":
            (this.state.currentState==4?"Order is deliverd.":""))))}</Text>
        </View>
        </>
